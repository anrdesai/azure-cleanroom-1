...
timeout 20 bash -c `
  "until curl -sS -X GET localhost:$port/members | jq -r '.value[].status' | grep Accepted > /dev/null; do echo Waiting for member to be in Accepted state...; sleep 5; done"
...
timeout 20 bash -c `
  "until curl -sS -X GET localhost:$port/members | jq -r '.value[].status' | grep Active > /dev/null; do echo Waiting for member to be in Active state...; sleep 5; done"
curl -sS -X GET localhost:$port/members | jq
