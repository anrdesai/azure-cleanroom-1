...
unformatted_files=""
badly_named_files=""
for file in $(find "$@" | grep -e '\.h$' -e '\.hpp$' -e '\.cpp$' -e '\.c$'); do
...
if [ "$unformatted_files" != "" ]; then
  ...
  if ! $fix ; then
    echo "Run 'cd $(git rev-parse --show-toplevel) && scripts/ci-checks.sh -f' to fix the formatting issues automatically or download the VS Code clang-format extension to fix issues on save."
    exit 1
  fi
else
