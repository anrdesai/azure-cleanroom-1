...
pwsh $collabSamplePath/db-access/build-application.ps1 -tag $tag -repo $repo -push

az cleanroom config add-application `
    --cleanroom-config $consumerConfig `
    --name demo-app `
    --image "$repo/db-access:$tag" `
    --command "./main" `
    --datasinks "consumer-db-output=/mnt/remote/output" `
