...
if ($repo) {
    $imageName = "$repo/workloads/kserve-inferencing-agent:$tag"
}
else {
...
Write-Host "Moving helmchart $HELMFILE to $sandbox_common/kserve-inferencing-agent.tgz"

Move-Item -Path $HELMFILE -Destination "$sandbox_common/kserve-inferencing-agent.tgz" -Force
Pop-Location

if ($push) {
    docker push $imageName

    $digest = Get-Digest -repo "$repo" -containerName "workloads/kserve-inferencing-agent" -tag $tag
    $digestNoPrefix = $digest.Split(":")[1]

    @"
kserve-inferencing-agent:
  version: $tag
  image: $repo/workloads/kserve-inferencing-agent@$digest
  helm: $repo/workloads/helm/kserve-inferencing-agent:$semanticVersion
"@ | Out-File "$sandbox_common/version.yaml"

    Push-Location $sandbox_common
    oras push "$repo/versions/workloads/kserve-inferencing-agent:$digestNoPrefix,latest" ./version.yaml
    helm push ./kserve-inferencing-agent.tgz "oci://$repo/workloads/helm"
    Pop-Location
}