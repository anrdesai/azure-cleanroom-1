...
import { SigningAlgorithm, ccf } from "@microsoft/ccf-app/global";
...
export function getTenantId(memberId: string): string {
  ...
  return info.member_data != null ? info.member_data.tenant_id : "";
}
...
export function verifySignature(sign: Sign, data: ArrayBuffer) {
  ...
  const algorithmName: string = isCcfv4() ? "RSASSA-PKCS1-v1_5" : "RSA-PSS";
...
export function isCcfv4(): boolean {
