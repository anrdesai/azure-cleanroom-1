...
namespace httplib
{
  ...
  public:
    ...
    bool operator()(
      MultipartContentHeader header, ContentReceiver receiver) const
    {
  ...
  struct Request
  {
    ...
    std::string get_header_value(
      const std::string& key, const char* def = "", size_t id = 0) const;
    uint64_t get_header_value_u64(
      const std::string& key, uint64_t def = 0, size_t id = 0) const;
  ...
  struct Response
  {
    ...
    void set_redirect(
      const std::string& url, int status = StatusCode::Found_302);
  ...
  namespace detail
  {
     ...
     * /:
     * "/begin/(.*)/end" will match both "/begin/middle/end" and
     * "/begin/1/2/end".
     */
  ...
  public:
    ...
    bool set_base_dir(
      const std::string& dir, const std::string& mount_point = std::string());
  ...
  private:
    ...
    bool handle_file_request(
      const Request& req, Response& res, bool head = false);
    bool dispatch_request(
      Request& req, Response& res, const Handlers& handlers) const;
    ...
    bool write_response(
      Stream& strm, bool close_connection, Request& req, Response& res);
  ...
  public:
    ...
    Result Get(
      const std::string& path, const Headers& headers, Progress progress);
    ...
    Result Post(
      const std::string& path, const Headers& headers, const Params& params);
    Result Post(
      const std::string& path,
      const Headers& headers,
      const Params& params,
    ...
    X509_STORE* create_ca_cert_store(
      const char* ca_cert, std::size_t size) const;
#endif
  ...
  protected:
    ...
    bool write_content_with_provider(
      Stream& strm, const Request& req, Error& error) const;
  ...
  private:
    ...
    bool read_response_line(
      Stream& strm, const Request& req, Response& res) const;
    bool write_request(
      Stream& strm, Request& req, bool close_connection, Error& error);
  ...
  private:
    ...
    bool connect_with_proxy(
      Socket& sock, Response& res, bool& success, Error& error);
  ...
  namespace detail
  {
    ...
    inline uint64_t get_header_value_u64(
      const Headers& headers, const std::string& key, uint64_t def, size_t id)
    {
  ...
  inline uint64_t Request::get_header_value_u64(
    const std::string& key, uint64_t def, size_t id) const
  {
    return detail::get_header_value_u64(headers, key, def, id);
  }

  inline uint64_t Response::get_header_value_u64(
    const std::string& key, uint64_t def, size_t id) const
  {
    return detail::get_header_value_u64(headers, key, def, id);
  }
...
#  endif
  ...
  namespace detail
  {
    ...
    public:
      ...
      virtual bool compress(
        const char* data, size_t data_length, bool last, Callback callback) = 0;
    };
    ...
    public:
      ...
      virtual bool decompress(
        const char* data, size_t data_length, Callback callback) = 0;
    };
    ...
    public:
      ...
      bool decompress(
        const char* data, size_t data_length, Callback callback) override;
    ...
    public:
      stream_line_reader(
        Stream& strm, char* fixed_buffer, size_t fixed_buffer_size);
  ...
  namespace detail
  {
    ...
    inline bool from_hex_to_i(
      const std::string& s, size_t i, size_t cnt, int& val)
    {
      if (i >= s.size())
      {
        return false;
    ...
    inline std::string encode_query_param(const std::string& value)
    {
      ...
      {
        if (
          std::isalnum(static_cast<uint8_t>(c)) || c == '-' || c == '_' ||
          c == '.' || c == '!' || c == '~' || c == '*' || c == '\'' ||
          c == '(' || c == ')')
        {
          escaped << c;
    ...
    inline std::string decode_url(
      const std::string& s, bool convert_plus_to_space)
    {
      std::string result;
    ...
    inline std::pair<size_t, size_t> trim(
      const char* b, const char* e, size_t left, size_t right)
    {
...
#  endif
    ...
    inline ssize_t send_socket(
      socket_t sock, const void* ptr, size_t size, int flags)
    {
      return handle_EINTR([&]() {
        return send(
          sock,
...
#  endif
    ...
    inline Error wait_until_socket_is_ready(
...
#  endif
    ...
    inline std::string escape_abstract_namespace_unix_domain(
      const std::string& s)
    {
      if (s.size() > 1 && s[0] == '\0')
      {
...
#  endif
        ...
        if (sock != INVALID_SOCKET)
        {
          ...
          std::copy(
            unescaped_host.begin(), unescaped_host.end(), addr.sun_path);
...
#  ifdef SOCK_CLOEXEC
        auto sock = socket(
          rp->ai_family, rp->ai_socktype | SOCK_CLOEXEC, rp->ai_protocol);
#  else
...
#  define USE_IF2IP
    ...
    inline void get_remote_ip_and_port(
      socket_t sock, std::string& ip, int& port)
    {
      struct sockaddr_storage addr;
      socklen_t addr_len = sizeof(addr);
...
#  endif
    ...
    inline constexpr unsigned int str2tag_core(
      const char* s, size_t l, unsigned int h)
    {
      return (l == 0) ?
        h :
        str2tag_core(
    ...
    inline bool nocompressor::compress(
      const char* data, size_t data_length, bool /*last*/, Callback callback)
    {
      if (!data_length)
      {
        return true;
    ...
    inline gzip_decompressor::gzip_decompressor()
    {
      ...
      // to ensure that any gzip stream can be decoded. The offset of 32
      // specifies that the stream type should be automatically detected either
      // gzip or deflate.
    ...
    inline bool gzip_decompressor::decompress(
      const char* data, size_t data_length, Callback callback)
    {
      assert(is_valid_);
    ...
    inline bool read_content_without_length(
      Stream& strm, ContentReceiverWithProgress out)
    {
      char buf[CPPHTTPLIB_RECV_BUFSIZ];
      uint64_t r = 0;
      for (;;)
    ...
    inline bool read_content_chunked(
      Stream& strm, T& x, ContentReceiverWithProgress out)
    {
      ...
      // received, possibly followed by a trailer section, and finally
      ...
      // does't care for the existence of the final CRLF. In other words, it
      ...
      return prepare_content_receiver(
        ...
        [&](const ContentReceiverWithProgress& out) {
          ...
          else
          {
            ...
            {
              ret =
                read_content_with_length(strm, len, std::move(progress), out);
            }
          }
    ...
    inline ssize_t write_request_line(
      Stream& strm, const std::string& method, const std::string& path)
    {
      std::string s = method;
      s += " ";
      s += path;
      ...
      Error& error)
    {
      ...
      if (
        res.status == StatusCode::SeeOther_303 &&
    ...
    inline void parse_query_text(
      const char* data, std::size_t size, Params& params)
    {
    ...
    inline void parse_disposition_params(const std::string& s, Params& params)
    {
      ...
      split(
        s.data(), s.data() + s.size(), ';', [&](const char* b, const char* e) {
          ...
          {
            params.emplace(
              trim_double_quotes_copy((key)), trim_double_quotes_copy((val)));
          }
        });
    }
      ...
      Range r, size_t content_length)
    {
      ...
      assert(
        r.first <= r.second && r.second < static_cast<ssize_t>(content_length));
      (void)(content_length);
      return std::make_pair(
        r.first, static_cast<size_t>(r.second - r.first) + 1);
    }
...
#    if TARGET_OS_OSX
    template <typename T>
    using CFObjectPtr = std::
      unique_ptr<typename std::remove_pointer<T>::type, void (*)(CFTypeRef)>;
    ...
    inline bool retrieve_root_certs_from_keychain(
      CFObjectPtr<CFArrayRef>& certs)
    {
    ...
    inline bool add_certs_to_x509_store(CFArrayRef certs, X509_STORE* store)
    {
      ...
      for (auto i = 0; i < CFArrayGetCount(certs); ++i)
      {
        ...
        if (
          SecItemExport(cert, kSecFormatX509Cert, 0, nullptr, &cert_data) !=
          errSecSuccess)
        {
          continue;
        }
...
#  endif // _WIN32
      ...
      bool is_proxy = false)
    {
      ...
      std::string response;
      {
        ...
        {
          response =
            H(H(A1) + ":" + auth.at("nonce") + ":" + nc + ":" + cnonce + ":" +
              qop + ":" + H(A2));
        }
      }
        ...
        if (pos != std::string::npos)
        {
          ...
          else if (type == "Digest")
          {
            ...
            for (auto i = beg; i != std::sregex_iterator(); ++i)
            {
              ...
              auto val = m.length(2) > 0 ? s.substr(
  ...
  inline std::string append_query_params(
    const std::string& path, const Params& params)
  {
  ...
  inline std::string Request::get_header_value(
    const std::string& key, const char* def, size_t id) const
  {
  ...
  inline void Request::set_header(
    const std::string& key, const std::string& val)
  {
  ...
  inline std::string Request::get_param_value(
    const std::string& key, size_t id) const
  {
  ...
  inline std::string Response::get_header_value(
    const std::string& key, const char* def, size_t id) const
  {
    return detail::get_header_value(headers, key, def, id);
  }
  ...
  inline void Response::set_header(
    const std::string& key, const std::string& val)
  {
    if (!detail::has_crlf(key) && !detail::has_crlf(val))
    {
  ...
  inline void Response::set_content(
    const char* s, size_t n, const std::string& content_type)
  {
    body.assign(s, n);
  ...
  inline void Response::set_content(
    const std::string& s, const std::string& content_type)
  {
    set_content(s.data(), s.size(), content_type);
  }

  inline void Response::set_content(
    std::string&& s, const std::string& content_type)
  {
    body = std::move(s);
  ...
  inline void Response::set_file_content(
    const std::string& path, const std::string& content_type)
  {
  ...
  inline std::string Result::get_request_header_value(
    const std::string& key, const char* def, size_t id) const
  {
  ...
  namespace detail
  {
    ...
    inline void SocketStream::get_remote_ip_and_port(
      std::string& ip, int& port) const
    {
      return detail::get_remote_ip_and_port(sock_, ip, port);
    }

    inline void SocketStream::get_local_ip_and_port(
      std::string& ip, int& port) const
    {
    ...
    inline void BufferStream::get_remote_ip_and_port(
      std::string& /*ip*/, int& /*port*/) const
    {}

    inline void BufferStream::get_local_ip_and_port(
      std::string& /*ip*/, int& /*port*/) const
    {}
  ...
  inline bool Server::set_base_dir(
    const std::string& dir, const std::string& mount_point)
  {
    return set_mount_point(mount_point, dir);
  }

  inline bool Server::set_mount_point(
    const std::string& mount_point, const std::string& dir, Headers headers)
  {
  ...
  inline Server& Server::set_error_handler_core(
    HandlerWithResponse handler, std::true_type)
  {
  ...
  inline bool Server::bind_to_port(
    ...
    bool need_apply_ranges)
  {
    ...
    if (
      res.body.empty() && !res.content_length_ && !res.content_provider_ &&
  ...
  inline bool Server::handle_file_request(
    const Request& req, Response& res, bool head)
  {
  ...
  inline int Server::bind_internal(
    const std::string& host, int port, int socket_flags)
  {
    if (is_decommisioned)
    {
  ...
  inline bool Server::routing(Request& req, Response& res, Stream& strm)
  {
    ...
    if (
      (req.method == "GET" || is_head_request) &&
...
#  endif
  ...
  inline bool ClientImpl::create_and_connect_socket(
    ...
    Error& error)
  {
    ...
    // Body
    if (
      (res.status != StatusCode::NoContent_204) && req.method != "HEAD" &&
      req.method != "CONNECT")
    {
  ...
  inline ContentProviderWithoutLength ClientImpl::
      ...
      const MultipartFormDataProviderItems& provider_items) const
  {
    ...
    return [&, cur_item, cur_start](
             size_t offset, DataSink& sink) mutable -> bool {
  ...
  inline Result ClientImpl::Post(
    const std::string& path, const MultipartFormDataItems& items)
  {
  ...
  inline Result ClientImpl::Options(
    const std::string& path, const Headers& headers)
  {
    Request req;
    req.method = "OPTIONS";
 ...
 */
  ...
  namespace detail
  {
    ...
    inline void ssl_delete(
      std::mutex& ctx_mutex, SSL* ssl, socket_t sock, bool shutdown_gracefully)
    {
...
#  endif
    ...
    inline void SSLSocketStream::get_local_ip_and_port(
      std::string& ip, int& port) const
    {
  ...
  inline SSLServer::SSLServer(
    X509* cert, EVP_PKEY* private_key, X509_STORE* client_ca_cert_store)
  {
    ctx_ = SSL_CTX_new(TLS_server_method());
  ...
  inline void SSLServer::update_certs(
    X509* cert, EVP_PKEY* private_key, X509_STORE* client_ca_cert_store)
  {
  ...
  inline void SSLClient::load_ca_cert_store(
  ...
  inline bool SSLClient::connect_with_proxy(
    Socket& socket, Response& res, bool& success, Error& error)
  {
    success = true;
    Response proxy_res;
