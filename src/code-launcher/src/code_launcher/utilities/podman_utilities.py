...
async def start_application_container(
    ...
    if application.datasources:
        for key in application.datasources.keys():
            ...
            mountpoint_utilities.wait_for_mount_point(key)
    ...
    if create_container:
        if application.datasources:
            for key, value in application.datasources.items():
                datasource_mount_path = mountpoint_utilities.get_mount_path(key)
                mounts.append(
                    {
                        "type": "bind",
        ...
        if application.datasinks:
            for key, value in application.datasinks.items():
                datasink_mount_path = mountpoint_utilities.get_mount_path(key)
                mounts.append(
                    {
                        "type": "bind",
