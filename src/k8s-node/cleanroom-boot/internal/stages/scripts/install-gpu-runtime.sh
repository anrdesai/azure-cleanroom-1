...
#   - containerd already installed and config migrated
#     (by the flex-node agent stage of cleanroom-boot).
#
# Must run AFTER the flex-node agent stage so that containerd is
# available and its config has been migrated to the correct format.
set -e
...
# Wait for containerd to be fully ready after restart. The ctr commands below
# will fail with EOF if containerd's gRPC socket isn't accepting connections yet.
...
for i in $(seq 1 30); do
  ...
  if [ "$i" -eq 30 ]; then
    echo "ERROR: containerd did not become ready within 30 seconds." >&2
    exit 1
  fi
  sleep 1
done
...
#
# GFD writes labels to a local file that NFD (Node Feature Discovery)
# reads via its "local" source. This avoids needing API server access
# from GFD — NFD handles the node labeling. The output directory
# /etc/kubernetes/node-feature-discovery/features.d/ is mounted into the
# NFD worker pod by the GPU Operator's NFD sub-chart.
# =============================================================================
